...
class RangeInfo:
    def __init__(self, destination_range_start, source_range_start, range_length):
        ...
        self.source_to_destination_diff = self.destination_range_start - self.source_range_start

    def __contains__(self, item):
        return self.source_range_start <= item <= self.source_range_start + self.range_length

    def __getitem__(self, item):
        if item in self:
            return item + self.source_to_destination_diff
        else:
            raise KeyError(item)
...
class SourceDestinationMap:
    def __init__(self, name: str):
        self.name = name
        self.mapping_ranges: List[RangeInfo] = []

    def add_range(self, destination_range_start, source_range_start, range_length):
        range_info = RangeInfo(destination_range_start, source_range_start, range_length)
        self.mapping_ranges.append(range_info)

    def __getitem__(self, item):
        for range_info in self.mapping_ranges:
            if item in range_info:
                return range_info[item]
        return item  # Any source numbers that aren't mapped correspond to the same destination number

    def __repr__(self):
        return f"{self.name}\n{self.mapping_ranges}"


simple_seeds = [79, 14, 55, 13]
seeds = 1367444651, 99920667, 3319921504, 153335682, 67832336, 139859832, 2322838536, 666063790, 1591621692, 111959634, \
            442852010, 119609663, 733590868, 56288233, 2035874278, 85269124, 4145746192, 55841637, 864476811, 347179760,


def create_source_destination_map(name, range_infos: List[Tuple]) -> SourceDestinationMap:
    source_destination_map = SourceDestinationMap(name)
    for ri in range_infos:
        source_destination_map.add_range(ri[0], ri[1], ri[2])
    return source_destination_map


simple_seed_to_soil_mappings = create_source_destination_map("seed_to_soil", [
    (50, 98, 2),
    (52, 50, 48),
])
simple_soil_to_fertilizer_mappings = create_source_destination_map("soil_to_fertilizer", [
    (0, 15, 37,),
    (37, 52, 2,),
    (39, 0, 15,),
])
simple_fertilizer_to_water_mappings = create_source_destination_map("fertilizer_to_water", [
    (49, 53, 8,),
    (0, 11, 42,),
    (42, 0, 7,),
    (57, 7, 4,),
])
simple_water_to_light_mappings = create_source_destination_map("water_to_light", [
    (88, 18, 7,),
    (18, 25, 70,),
])
simple_light_to_temperature_mappings = create_source_destination_map("light_to_temperature", [
    (45, 77, 23,),
    (81, 45, 19,),
    (68, 64, 13,),
])
simple_temperature_to_humidity_mappings = create_source_destination_map("temperature_to_humidity", [
    (0, 69, 1,),
    (1, 0, 69,),
])
simple_humidity_to_location_mappings = create_source_destination_map("humidity_to_location", [
    (60, 56, 37,),
    (56, 93, 4,),
])

seed_to_soil_mappings = create_source_destination_map("seed_to_soil", [
...
soil_to_fertilizer_mappings = create_source_destination_map("soil_to_fertilizer", [
...
fertilizer_to_water_mappings = create_source_destination_map("fertilizer_to_water", [
...
water_to_light_mappings = create_source_destination_map("water_to_light", [
...
light_to_temperature_mappings = create_source_destination_map("light_to_temperature", [
...
temperature_to_humidity_mappings = create_source_destination_map("temperature_to_humidity", [
...
humidity_to_location_mappings = create_source_destination_map("humidity_to_location", [
...
def part1():
    ...
    for seed in simple_seeds:
        soil = simple_seed_to_soil_mappings[seed]
        fertilizer = simple_soil_to_fertilizer_mappings[soil]
        water = simple_fertilizer_to_water_mappings[fertilizer]
        light = simple_water_to_light_mappings[water]
        temperature = simple_light_to_temperature_mappings[light]
        humidity = simple_temperature_to_humidity_mappings[temperature]
        location = simple_humidity_to_location_mappings[humidity]
        simple_locations.append(location)

    locations = []
    for seed in seeds:
        soil = seed_to_soil_mappings[seed]
        fertilizer = soil_to_fertilizer_mappings[soil]
        water = fertilizer_to_water_mappings[fertilizer]
        light = water_to_light_mappings[water]
        temperature = light_to_temperature_mappings[light]
        humidity = temperature_to_humidity_mappings[temperature]
        location = humidity_to_location_mappings[humidity]
        locations.append(location)
    return min(locations)


def part2():
    lowest_location = 1000000000000000000000000000000
    idx = 0
    while idx < len(seeds):
        print(f"{idx/2}/{len(seeds)/2}")
        seed_start = seeds[idx]
        seed_stop = seed_start + seeds[idx + 1]
        for seed in range(seed_start, seed_stop):
            if seed % 1_000_000 == 0:
                print(f"{round((seed-seed_start) / (seed_stop-seed_start), 2)} %")
            soil = seed_to_soil_mappings[seed]
